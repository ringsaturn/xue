...
- Satellite (`himawari`, `goeseast`, `goeswest`, `meteosat`): Himawari-9's
  ...
  Beside the guns, the three NOAA sources publish the **DEBRA dust
  confidence** `dustcf` (`DebraProducer`, `docs/satellite.md` §"The
  DEBRA confidence"): Miller et al. 2017 as `shachen` implements it, the
  chain composed from its per-equation modules with the ABI retune, on
  five channels (`ir039`, `wv062`, `ir086`, `ir104`, `ir123`; the sources
  fetch six) and two ancillary fields (`satellite/ancillary.py`): GFS
  `TMP:surface` fetched per slot by byte range and cached under
  `data/raw/ancillary/gfs/`, and the CAMEL emissivity months the
  operational DEBRA pipeline stages on the bucket under
  `shachen-ops/ancillary/camel/<region>/<YYYYMM>.nc`, mirrored by
  `make pull-r2-ancillary` (`ANCILLARY=true` in the rounds script; a
  region lacking the month falls back to its newest staged month with a
  warning). Defined over water and over land inside a staged file, no
  data elsewhere (staging a region extends the product); `cf_comb` with
  the split-window gate (`dt1 = dt2 = 0` → 0); a composite bundle of one
  variable (`COMPOSITE_BUNDLES["dustcf"] = ("dustcf",)`), GRIB2 3/192/4
  on surface 8 with the `producer` block, the guns' codebook. Producers
  take `slot` and `grid` and resolve their ancillaries through
  `ancillary_for(platform, slot, root)`; `tests/test_debra.py` holds the
  chain equal to `shachen.pipeline.run_debra` cell for cell. Meteosat
  publishes no `dustcf` (no staged region on its disk).
  `SATELLITE_VARIABLE_IDS` (channels + guns + the confidence) and
  ...
  `tests/test_satellite.py` runs the twenty-four fixture tiles in
  `tests/fixtures/himawari/` (six bands, two scans) through the whole
  stage, the producers and both encoders; `tests/test_goes.py` the twelve
...
- `gdalinfo` has a second source. `gdal.dataset_info` is the one entry
  ...
  one reason for the `xuepy>=0.20` floor) when the build converts natively,

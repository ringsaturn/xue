...
| Source | `--model` | Grid and axis | Newest run |
|---|---|---|---|
| NOAA GFS 0.25° | `gfs` | 1440 × 721, 0.25°; hourly to F120, 3-hourly to F240 (161 frames) | [![the newest run](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fdataset.ringsaturn.me%2Fxue%2Flatest.json&query=%24.run&label=run&color=0b7cbd&style=for-the-badge&cacheSeconds=600)](https://dataset.ringsaturn.me/xue/latest.json) |
| GFS surface flux | `sflux` | 3072 × 1536 Gaussian, ~13 km; as GFS | [![the newest run](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fdataset.ringsaturn.me%2Fxue%2Flatest-sflux.json&query=%24.run&label=run&color=2b6cb0&style=for-the-badge&cacheSeconds=600)](https://dataset.ringsaturn.me/xue/latest-sflux.json) |
...
(`latest.json` for GFS, `latest-<model>.json` for the rest, which the
badge links to): the cycle as `YYYYMMDDHH` UTC for a forecast, and for a

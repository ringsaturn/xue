#!/bin/sh
# The live MRMS feed: one round of the rolling window every ROUND_MINUTES,
# looped until UNTIL_MINUTE past the hour this job started in, so an hourly
# cron job hands the feed to the next without a gap or an overlap
# (.github/workflows/publish-mrms.yml owns the schedule; this script owns
# the rounds).
#
# A round is: the bucket's newest frame against the round that is live
...
# step's seconds.
#
# Environment: ROUND_MINUTES (5), UNTIL_MINUTE (55: the last round of the
# hour starts no later than this minute), HOURS (4: the
# three-hour window plus the hour in progress), PROFILE (balanced), KEEP (2),
# FORCE (true builds a round even when the live one is current), DRY_RUN
# (--dryrun previews the uploads and prunes), ONCE (true runs one round and
# exits — a manual check). PYTHON names the interpreter (the Makefile's
...
while :; do
  ...
  newest=$($python -c "from xuebuild.fetch import latest_mrms_slot; from xuebuild.sources import source_spec; print(latest_mrms_slot(source_spec('$model')).strftime('%Y-%m-%dT%H:%M:%SZ'))") || ok=false
  ...
  if [ "$ok" = true ] && [ "$newest" = "$live" ] && [ "$force" != true ] && [ -z "$dry_run" ]; then
    ...
    if $python -m xuebuild -v build-bin --model $model --run latest --hours "$hours" \
      ...
      if make upload-r2 MODEL=$model RUN="$run" ROUND="$round" DRY_RUN="$dry_run"; then
        ...
        # they can go once a round of the new run has fetched), and the
        # round just uploaded.

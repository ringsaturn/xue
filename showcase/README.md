...
A **case** is one past forecast run, cropped to the region and hours of a
single weather event, and published as a permanent dataset alongside the live
feed. Cases are what `showcase.html` lists and what `/?case=<id>` plays back.
...
1. Write `showcase/cases/<id>.json` (the filename must match the `id`):
   ...
   - `variables` is any subset of what the model publishes, in any order —
     `tmp2m`, `prate`, `wind10m`, and `dswrf` on sflux only. The manifest ships
     just these, and the viewer hides the buttons for the rest.

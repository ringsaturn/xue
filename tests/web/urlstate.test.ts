...
describe("parseModelFromSearch", () => {
  it("resolves served models and their aliases, case-insensitively", () => {
    ...
    expect(parseModelFromSearch("?model=aifs")).toBe("aifs");
    expect(parseModelFromSearch("?model=AIFS-Single")).toBe("aifs");
    expect(parseModelFromSearch("?model=ifshres")).toBe("ifshres");
    expect(parseModelFromSearch("?model=HRES")).toBe("ifshres");
    expect(parseModelFromSearch("?model=ifs9km")).toBe("ifshres");
    expect(parseModelFromSearch("?model=cfs")).toBe("cfs");
    expect(parseModelFromSearch("?model=CFSv2")).toBe("cfs");
    ...
    expect(parseModelFromSearch("?model=himawari")).toBe("himawari");
    expect(parseModelFromSearch("?model=Himawari-9")).toBe("himawari");
    expect(parseModelFromSearch("?model=goeseast")).toBe("goeseast");
    expect(parseModelFromSearch("?model=GOES-19")).toBe("goeseast");
    expect(parseModelFromSearch("?model=goes-west")).toBe("goeswest");
    expect(parseModelFromSearch("?model=goes18")).toBe("goeswest");
    expect(parseModelFromSearch("?model=meteosat")).toBe("meteosat");
    expect(parseModelFromSearch("?model=Meteosat-12")).toBe("meteosat");
    expect(parseModelFromSearch("?model=mtg")).toBe("meteosat");
    expect(parseModelFromSearch("?model=MTG-I1")).toBe("meteosat");
    expect(parseModelFromSearch("?model=geo")).toBe("geo");
    expect(parseModelFromSearch("?model=mosaic")).toBe("geo");
    expect(parseModelFromSearch("?model=geostationary")).toBe("geo");
...
describe("parseBackendFromSearch", () => {
  it("is the store unless the URL asks for the container", () => {
    expect(parseBackendFromSearch("")).toBe("zarr");
    expect(parseBackendFromSearch("?model=gfs&type=temp")).toBe("zarr");
    expect(parseBackendFromSearch("?backend=zarr")).toBe("zarr");
  });

  it("accepts xue case-insensitively", () => {
    expect(parseBackendFromSearch("?backend=xue")).toBe("xue");
    expect(parseBackendFromSearch("?backend=Xue")).toBe("xue");
    expect(parseBackendFromSearch("?model=gfs&backend=xue&type=temp")).toBe("xue");
  });

  it("treats any other value as the store rather than erroring", () => {
    expect(parseBackendFromSearch("?backend=")).toBe("zarr");
    expect(parseBackendFromSearch("?backend=hdf5")).toBe("zarr");
    expect(parseBackendFromSearch("?backend=true")).toBe("zarr");
  });

  it("survives a layer switch, which preserves unrelated params", () => {
    expect(parseBackendFromSearch(searchForVariable("tmp2m", "?backend=xue&type=precip"))).toBe("xue");
  });
});

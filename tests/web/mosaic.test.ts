...
import { FORECAST_MODELS, mosaicMembers } from "../../web/src/manifest";
import {
...
describe("the geo entry", () => {
  ...
  it("opens the members this build knows and skips one it predates", () => {
    // Meteosat is listed first so it takes its band from the 0° slot; its
    // hourly cycle is the coarsest cadence, so it never drives the timeline.
    const ids = mosaicMembers("geo").map((member) => member.id);
    expect(ids).toEqual(["meteosat", "himawari", "goeswest", "goeseast"]);
    for (const id of ids) {
      expect(FORECAST_MODELS[id].subLongitude).toBeDefined();
      expect(FORECAST_MODELS[id].cadenceSeconds).toBe(id === "meteosat" ? 3600 : 600);

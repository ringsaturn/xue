...
describe("the satellite registry", () => {
  ...
  it("paints the disk's edge as nothing and cloud tops bright and cold in colour", () => {
    ...
    // Warm clear sea near 300 K is dark and opaque; a high cloud near 240 K
    // is light grey; a convective top near 200 K is enhanced in colour.
    const at = (kelvin: number) => rgba(palette, Math.round((kelvin - 180) / 0.6));
    expect(at(300)[3]).toBe(255);
    expect(at(300)[0]).toBeLessThan(80);
    expect(at(243)[0]).toBeGreaterThan(200);
    const [r, g, b, a] = at(210);
    expect(a).toBe(255);
    expect(Math.max(r, g, b) - Math.min(r, g, b)).toBeGreaterThan(100);
    expect(decodeValue(variable, 253)).toBeCloseTo(331.8, 6);
    expect(decodeValue(variable, 255)).toBeNull();
  });

  it("reads its legend in kelvin over the codebook", () => {
    ...
    expect(isobaricLegend(identity)).toEqual(["330", "300", "270", "240", "210", "180"]);
    expect(variableSpec("ir104")?.legend()).toEqual(["330", "300", "270", "240", "210", "180"]);

...
export type ForecastModelId = "gfs" | "ecmwf" | "aifs" | "sflux" | "hrrr" | "cma" | "mrms" | "jma" | "himawari" | "goeseast" | "goeswest" | "geo";
...
export const FORECAST_MODELS: Record<ForecastModelId, ForecastModelInfo> = {
  ...
  geo: {
    ...
    mosaic: true,
    members: ["meteosat" as ForecastModelId, "himawari", "goeswest", "goeseast"],
  },
};
...
/** The model switch's entries, in order: the five forecasts, the six
 * rolling observation windows (MRMS, the JMA nowcast, the CMA mosaic and
 * the three geostationary imagers) and the geostationary mosaic, a view
 * over the imagers with no feed of its own. */
export const FORECAST_MODEL_IDS: readonly ForecastModelId[] = ["gfs", "sflux", "ecmwf", "aifs", "hrrr", "mrms", "jma", "cma", "himawari", "goeseast", "goeswest", "geo"];

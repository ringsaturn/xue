...
| CMA-RADAR | the NetCDF observation path: unscaling, the fill value, a `unitSeconds: 360` axis listing its offsets around archive gaps, `--hours`; since 0.17 a fetched window like JMA's, read out of the cma-radar archive with the window's first hour as the run (`tests/test_cma.py`); the id is `cma` since then, `radar` being the local-file shape the wheels before 0.17 know |
